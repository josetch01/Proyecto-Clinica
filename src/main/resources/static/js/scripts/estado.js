const url = "/sistemaginecofem/api/estados/";
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $.each(js, function (index, value) {
                t.row.add([value.id, value.nombre, botonera]);
            });
            t.draw(false);
        },
    }).fail(function () {
...
function getFila(id) {
    $.ajax({
        ...
        timeout: 600000,
        success: function (data) {
            $("#modal-title").text("Editar Estado de Cita");
...
function clear() {
    $("#modal-title").text("Nuevo Estado de Cita");
    $("#nombre").val("");
    $("#guardar").data("id", 0);
